...
def resume():
    ...
    index = 0 path = os.path.abspath(os.path.join(os.path.dirname(__file__),
    'hasil_uji')) for s in sentences: listterm = [] term_fn = []
    sentence_result = '' original_result = '' pre_result =
    preprocessing.preprocessing(s) listterm = vsm.list_term(pre_result)
    term_frequency = vsm.cal_tf(pre_result, listterm) term_frequency_judul =
    vsm.cal_tf_basic(pre_judul[index], listterm) term_frequency_normalize =
    vsm.cal_tf_normalize(term_frequency) term_frequency_normalize_judul =
    np.log(1 + np.asarray(term_frequency_judul)) document_frequency =
    vsm.cal_df(pre_result, listterm) document_frequency_judul =
    vsm.cal_df(pre_judul, listterm) for idx, tfn in
    enumerate(term_frequency_normalize): term_fn.append(tfn.tolist()) weight =
    vsm.cal_vsm(term_fn, document_frequency) weight_judul =
    vsm.cal_vsm_basic(term_frequency_normalize_judul,
    document_frequency_judul) index_sentence = svd.cal_svd(weight,
    weight_judul) for i in index_sentence: sentence_result += s[i] + ' ' for
    sen in s: original_result += sen sumarize.append(sentence_result)
    original.append(original_result) f = open(path + "/berita-" + str(index) +
    ".txt", "w+") f.write(sentence_result) print(index) index += 1
...
@app.route('/ringkasan')
def ringkasan():
    return render_template('ringkasan.html',username=session['username'])


@app.route('/pengujian')
@cross_origin(origin='*',headers=['Content-Type','Authorization'])
def pengujian():
    # allData = mongo.db.pengujian.find()
    # data = []
    # for i in allData:
    #     data.append(i)
    #     print(i)
